...
https://portal.nutanix.com/page/documents/details?targetId=CSI-Volume-Driver-v3_8:CSI-Volume-Driver-v3_8

## Features list

- Nutanix CSI Driver v3.8.0
...
```console
wget https://github.com/nutanix/helm-releases/releases/download/nutanix-csi-storage-3.8.0/nutanix-csi-storage-3.8.0.tgz
```

After downloading the chart you can directly set the values and install:

```console
helm install nutanix-csi nutanix-csi-storage-3.8.0.tgz --set <key>=<value> -n <namespace of your choice>
```

Or you can unzip, set values and then install:

```console
tar -xvf nutanix-csi-storage-3.8.0.tgz

helm install nutanix-csi nutanix-csi-storage-3.8.0 -n <namespace of your choice>
```
...
```console
helm upgrade nutanix-csi nutanix-csi-storage-3.8.0.tgz --set <key>=<value> -n <namespace of your choice>
```
...
| `ntnxInitConfigMap.usePEFallback` | Enables CSI pod-failover paths in PC mode during PC downtime. This does not switch Pulse to PE identity; Pulse keeps `cluster_uuid` PC-based and may skip telemetry if PC UUID cannot be resolved. | `false` |
...
Release note: In `usePC=true` + `usePEFallback=true`, Pulse now falls back to node topology metadata for `cluster_uuid` only when PC is unreachable; auth/config errors still fail fast.
...
This chart can optionally install the CN-RWX operator (RWX / ReadWriteMany volumes) via Helm hook jobs.

To enable CN-RWX:
- Set `volumeRWXSupport.enabled=true`
- Set `volumeRWXSupport.volumesRWXChartsUrl` to a CN-RWX Helm chart tarball (`.tgz`) URL reachable from inside the cluster
...
volumeRWXSupport:
  enabled: true
  volumesRWXChartsUrl: "https://<internal-host>/nutanix-core-files-cluster-operator-<version>.tgz"
```

### Air-gapped / disconnected environments

For air-gapped clusters (no internet access from nodes/pods):

- Set `volumeRWXSupport.jobImage` to a tools image (includes `kubectl`, `helm`, and required utilities) and mirror it into your local registry.
- Host the CN-RWX Helm chart `.tgz` internally and set `volumeRWXSupport.volumesRWXChartsUrl` to that internal URL.

If `volumeRWXSupport.jobImage` is left empty, the hook jobs use the public `docker.io/nutanix/cn-rwx-installer` image, which already bundles all required tools (`kubectl`, `helm`, `wget`, `jq`, `tar`, `base64`) — no tools are downloaded at runtime. Since that default image is pulled from Docker Hub, air-gapped clusters must set `volumeRWXSupport.jobImage` to a mirrored copy.

Two simple ways to host the CN-RWX chart `.tgz`:

1. Upload the `.tgz` to an internal artifact repository (Artifactory/Nexus) and use its download URL.
2. Serve the `.tgz` from an internal HTTP(S) server reachable from the cluster.

Example (simple HTTP server):

```console
# On an internal host reachable by the Kubernetes cluster:
mkdir -p /srv/cn-rwx-charts
cp nutanix-core-files-cluster-operator-0.1.501.tgz /srv/cn-rwx-charts/
python3 -m http.server 8080 --directory /srv/cn-rwx-charts

# Then configure:
#   volumeRWXSupport.volumesRWXChartsUrl: "http://<host>:8080/nutanix-core-files-cluster-operator-0.1.501.tgz"
```

If the chart endpoint requires HTTP basic authentication, create a `kubernetes.io/dockerconfigjson` secret containing credentials for the chart host and reference it in `imagePullSecrets`. The installer job will attempt to reuse these credentials for the chart download (if none match, it will fall back to unauthenticated download).

Example air-gapped values snippet:
...
volumeRWXSupport:
  enabled: true
  volumesRWXChartsUrl: "https://<internal-host>/path/to/nutanix-core-files-cluster-operator-0.1.501.tgz"
  jobImage: "<local-registry>/nutanix-core/files-cluster-operator/cn-rwx-installer:<tag>"
```

Note: You must also mirror all container images referenced by the CN-RWX Helm chart itself (operator and CN-RWX pods) into your local registry, and configure the CN-RWX chart values accordingly.
