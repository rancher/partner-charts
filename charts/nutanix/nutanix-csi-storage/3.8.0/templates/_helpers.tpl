...
{{/*
Determines the secret management type.
Returns "csi" if secrets are managed by the Helm chart, "user" if managed by the user.
*/}}
{{- define "nutanix-csi-storage.secretManagementType" -}}
{{- if eq .Values.ntnxInitConfigMap.usePC true -}}
    ...
    {{- else -}}
        {{- print "user" -}}
    {{- end -}}
{{- else if eq .Values.ntnxInitConfigMap.usePETopology true -}}
    {{- if eq .Values.createPESecrets true -}}
        {{- print "csi" -}}
    {{- else -}}
        {{- print "user" -}}
    {{- end -}}
