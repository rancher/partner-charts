...
https://portal.nutanix.com/page/documents/details?targetId=CSI-Volume-Driver-v3_7:CSI-Volume-Driver-v3_7

## Features list

- Nutanix CSI Driver v3.7.0
...
```console
wget https://github.com/nutanix/helm-releases/releases/download/nutanix-csi-storage-3.7.0/nutanix-csi-storage-3.7.0.tgz
```

After downloading the chart you can directly set the values and install:

```console
helm install nutanix-csi nutanix-csi-storage-3.7.0.tgz --set <key>=<value> -n <namespace of your choice>
```

Or you can unzip, set values and then install:

```console
tar -xvf nutanix-csi-storage-3.7.0.tgz

helm install nutanix-csi nutanix-csi-storage-3.7.0 -n <namespace of your choice>
```
...
```console
helm upgrade nutanix-csi nutanix-csi-storage-3.7.0.tgz --set <key>=<value> -n <namespace of your choice>
```
...
volumeRWXSupport:
  enabled: true
  volumesRWXChartsUrl: "https://example.com/path/to/nutanix-core-files-cluster-operator-<version>.tgz"
```
...
If `volumeRWXSupport.jobImage` is left empty, the hook jobs fall back to the legacy `alpine/k8s` image and download tools (Helm, packages) at runtime (not suitable for air-gapped environments).
...
volumeRWXSupport:
  enabled: true
  volumesRWXChartsUrl: "https://example.com/path/to/nutanix-core-files-cluster-operator-0.1.501.tgz"
