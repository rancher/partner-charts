...
spec:
  ...
  resources:
    requests:
      storage: {{ default $main_context.Values.global.persistence.size (index $main_context.Values.global.persistence $statefulContainer "size") }}
...
{{- range $skip, $service := $containerList }}
  ...
  {{- else if or (or (eq (include "basicauth.check" $main_context) "true") (or $main_context.Values.auth.oidcAuth.enabled (eq (include "check.dexAuth" $main_context) "true"))) $main_context.Values.features }}
    {{- $needsVolumesHeader = true }}
  {{- else if and (eq $service "config") ($main_context.Values.injectKanisterSidecar.enabled) }}
    {{- $needsVolumesHeader = true }}
...
{{- end }}
{{- if and (has "config" $containersInThisPod) (.Values.injectKanisterSidecar.enabled) }}
