...
kubectl delete crd environments.shipa.io --ignore-not-found=true
...
for NAMESPACE in $NAMESPACES; do
    ...
    IS_AUTODISCOVERY=$(kubectl get ns $NAMESPACE -o json | jq '.metadata.annotations' | jq 'has("autodiscovery.shipa.io/framework")')
    if [[ $IS_AUTODISCOVERY == "true" ]]; then
        echo "Skip removing namespace $NAMESPACE, as it is app autodiscovery namespace"
        continue
    fi
