...
echo "[INFO] Validating that shipa can be upgraded"

ENVIRONMENT_CRD=$(kubectl get crd environments.shipa.io --ignore-not-found)
if [[ -z $ENVIRONMENT_CRD ]]; then
  echo "[ERROR] Upgrade from [< 1.7.*] to [> 1.8.*] version is not supported"
  echo "[INFO]  Please do not upgrade on existing shipa 1.7.4 or older versions."
  echo "1.8.0 is meant to be installed fresh on new cluster, as it contains breaking changes that are complex to address in an update scenario."
  exit 1
fi
