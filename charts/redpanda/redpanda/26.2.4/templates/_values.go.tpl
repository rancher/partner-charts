...
{{- /* Transpiled by gotohelm from "github.com/redpanda-data/redpanda-operator/charts/redpanda/v25/chart/values.go" */ -}}
...
{{- define "redpanda.ExternalConfig.IsGatewayRequested" -}}
{{- $e := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (and (and $e.enabled (ne (toJson $e.gateway) "null")) $e.gateway.enabled)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "redpanda.ExternalConfig.IsGatewayEnabled" -}}
{{- $e := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (and (get (fromJson (include "redpanda.ExternalConfig.IsGatewayRequested" (dict "a" (list $e)))) "r") (gt ((get (fromJson (include "_shims.len" (dict "a" (list $e.gateway.parentRefs)))) "r") | int) (0 | int)))) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}

{{- define "redpanda.ExternalConfig.ValidateGateway" -}}
{{- $e := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- if (and (get (fromJson (include "redpanda.ExternalConfig.IsGatewayRequested" (dict "a" (list $e)))) "r") (eq ((get (fromJson (include "_shims.len" (dict "a" (list $e.gateway.parentRefs)))) "r") | int) (0 | int))) -}}
{{- $_ := (fail "external.gateway.enabled is true but external.gateway.parentRefs is empty; Gateway API mode requires at least one parentRef. Refusing to fall back to NodePort/LoadBalancer.") -}}
{{- end -}}
{{- end -}}
{{- end -}}

{{- define "redpanda.GatewayConfig.GatewayAdvertisedPort" -}}
{{- $g := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- if (ne (toJson $g.advertisedPort) "null") -}}
{{- $_is_returning = true -}}
{{- (dict "r" $g.advertisedPort) | toJson -}}
{{- break -}}
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (443 | int)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- break -}}
{{- end -}}
{{- $_ := (unset $result "apply_host_tuners") -}}
{{- $_is_returning = true -}}
{{- (dict "r" $result) | toJson -}}
{{- break -}}
{{- end -}}
...
{{- define "redpanda.HostTunerDefaults" -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
...
{{- range $_, $item := $items -}}
{{- $_1205___ok_11 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $seen $item.key false)))) "r") -}}
{{- $_ := (index $_1205___ok_11 0) -}}
{{- $ok_11 := (index $_1205___ok_11 1) -}}
{{- if $ok_11 -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1493_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil)))))) "r") -}}
{{- $cert := (index $_1493_cert_ok 0) -}}
{{- $ok := (index $_1493_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- define "redpanda.SASLAuth.GetMechanism" -}}
{{- $s := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- if (eq $s.mechanism "") -}}
...
{{- define "redpanda.ExternalListener.IsGatewayListener" -}}
{{- $l := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_is_returning = true -}}
{{- (dict "r" (eq (get (fromJson (include "_shims.ptr_Deref" (dict "a" (list $l.type "")))) "r") "tlsroute")) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_2071___ok_15 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v)))) "r") -}}
{{- $_ := ((index $_2071___ok_15 0) | float64) -}}
{{- $ok_15 := (index $_2071___ok_15 1) -}}
{{- if $ok_15 -}}
{{- $_ := (set $result $k $v) -}}
...
{{- $result := (dict) -}}
{{- range $k, $v := $c -}}
{{- $_2091_b_16_ok_17 := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" $v false)))) "r") -}}
{{- $b_16 := (index $_2091_b_16_ok_17 0) -}}
{{- $ok_17 := (index $_2091_b_16_ok_17 1) -}}
{{- if $ok_17 -}}
{{- $_ := (set $result $k $b_16) -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2136___hasAccessKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_access_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2136___hasAccessKey 0) -}}
{{- $hasAccessKey := (index $_2136___hasAccessKey 1) -}}
{{- $_2137___hasSecretKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_secret_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2137___hasSecretKey 0) -}}
{{- $hasSecretKey := (index $_2137___hasSecretKey 1) -}}
{{- $_2138___hasSharedKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_azure_shared_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2138___hasSharedKey 0) -}}
{{- $hasSharedKey := (index $_2138___hasSharedKey 1) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2174___containerExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_container" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2174___containerExists 0) -}}
{{- $containerExists := (index $_2174___containerExists 1) -}}
{{- $_2175___accountExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_storage_account" (coalesce nil))))) "r") -}}
{{- $_ := (index $_2175___accountExists 0) -}}
{{- $accountExists := (index $_2175___accountExists 1) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2180_value_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c `cloud_storage_cache_size` (coalesce nil))))) "r") -}}
{{- $value := (index $_2180_value_ok 0) -}}
{{- $ok := (index $_2180_value_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
{{- break -}}
