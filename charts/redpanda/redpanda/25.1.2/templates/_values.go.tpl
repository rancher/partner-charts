...
{{- $_is_returning := false -}}
{{- if (and (ne (toJson $rr.limits) "null") (ne (toJson $rr.requests) "null")) -}}
{{- $_419_cpuReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.requests) "cpu" "0")))) "r") -}}
{{- $cpuReq := (index $_419_cpuReq_ok 0) -}}
{{- $ok := (index $_419_cpuReq_ok 1) -}}
{{- if (not $ok) -}}
{{- $_421_cpuReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.limits) "cpu" "0")))) "r") -}}
{{- $cpuReq = (index $_421_cpuReq_ok 0) -}}
{{- $ok = (index $_421_cpuReq_ok 1) -}}
{{- end -}}
...
{{- $_is_returning := false -}}
{{- if (and (ne (toJson $rr.limits) "null") (ne (toJson $rr.requests) "null")) -}}
{{- $_445_cpuReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.requests) "cpu" "0")))) "r") -}}
{{- $cpuReq := (index $_445_cpuReq_ok 0) -}}
{{- $ok := (index $_445_cpuReq_ok 1) -}}
{{- if (not $ok) -}}
{{- $_447_cpuReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.limits) "cpu" "0")))) "r") -}}
{{- $cpuReq = (index $_447_cpuReq_ok 0) -}}
{{- $ok = (index $_447_cpuReq_ok 1) -}}
{{- end -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
...
{{- $_is_returning := false -}}
{{- if (and (ne (toJson $rr.limits) "null") (ne (toJson $rr.requests) "null")) -}}
{{- $_504_memReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.requests) "memory" "0")))) "r") -}}
{{- $memReq := (index $_504_memReq_ok 0) -}}
{{- $ok := (index $_504_memReq_ok 1) -}}
{{- if (not $ok) -}}
{{- $_506_memReq_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list ($rr.limits) "memory" "0")))) "r") -}}
{{- $memReq = (index $_506_memReq_ok 0) -}}
{{- $ok = (index $_506_memReq_ok 1) -}}
{{- end -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
...
{{- $_624_b_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $conf "cloud_storage_enabled" (coalesce nil))))) "r") -}}
{{- $b := (index $_624_b_ok 0) -}}
{{- $ok := (index $_624_b_ok 1) -}}
{{- $_is_returning = true -}}
...
{{- $_653_dir_7_ok_8 := (get (fromJson (include "_shims.typetest" (dict "a" (list "string" (index $values.config.node "cloud_storage_cache_directory") "")))) "r") -}}
{{- $dir_7 := (index $_653_dir_7_ok_8 0) -}}
{{- $ok_8 := (index $_653_dir_7_ok_8 1) -}}
{{- if $ok_8 -}}
{{- $_is_returning = true -}}
{{- (dict "r" $dir_7) | toJson -}}
{{- break -}}
...
{{- $_662_dir_9_ok_10 := (get (fromJson (include "_shims.typetest" (dict "a" (list "string" (index $tieredConfig "cloud_storage_cache_directory") "")))) "r") -}}
{{- $dir_9 := (index $_662_dir_9_ok_10 0) -}}
{{- $ok_10 := (index $_662_dir_9_ok_10 1) -}}
{{- if $ok_10 -}}
{{- $_is_returning = true -}}
{{- (dict "r" $dir_9) | toJson -}}
{{- break -}}
...
{{- $s := (toJson $t) -}}
{{- $tune := (fromJson $s) -}}
{{- $_808_m_ok := (get (fromJson (include "_shims.typetest" (dict "a" (list (printf "map[%s]%s" "string" "interface {}") $tune (coalesce nil))))) "r") -}}
{{- $m := (index $_808_m_ok 0) -}}
{{- $ok := (index $_808_m_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (dict)) | toJson -}}
{{- break -}}
...
{{- range $_, $item := $items -}}
{{- $_1026___ok_11 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $seen $item.key false)))) "r") -}}
{{- $_ := (index $_1026___ok_11 0) -}}
{{- $ok_11 := (index $_1026___ok_11 1) -}}
{{- if $ok_11 -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1314_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil)))))) "r") -}}
{{- $cert := (index $_1314_cert_ok 0) -}}
{{- $ok := (index $_1314_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_1823___ok_15 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v)))) "r") -}}
{{- $_ := ((index $_1823___ok_15 0) | float64) -}}
{{- $ok_15 := (index $_1823___ok_15 1) -}}
{{- if $ok_15 -}}
{{- $_ := (set $result $k $v) -}}
...
{{- $result := (dict) -}}
{{- range $k, $v := $c -}}
{{- $_1843_b_16_ok_17 := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" $v false)))) "r") -}}
{{- $b_16 := (index $_1843_b_16_ok_17 0) -}}
{{- $ok_17 := (index $_1843_b_16_ok_17 1) -}}
{{- if $ok_17 -}}
{{- $_ := (set $result $k $b_16) -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1888___hasAccessKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_access_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_1888___hasAccessKey 0) -}}
{{- $hasAccessKey := (index $_1888___hasAccessKey 1) -}}
{{- $_1889___hasSecretKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_secret_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_1889___hasSecretKey 0) -}}
{{- $hasSecretKey := (index $_1889___hasSecretKey 1) -}}
{{- $_1890___hasSharedKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_azure_shared_key" (coalesce nil))))) "r") -}}
{{- $_ := (index $_1890___hasSharedKey 0) -}}
{{- $hasSharedKey := (index $_1890___hasSharedKey 1) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1926___containerExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_container" (coalesce nil))))) "r") -}}
{{- $_ := (index $_1926___containerExists 0) -}}
{{- $containerExists := (index $_1926___containerExists 1) -}}
{{- $_1927___accountExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_storage_account" (coalesce nil))))) "r") -}}
{{- $_ := (index $_1927___accountExists 0) -}}
{{- $accountExists := (index $_1927___accountExists 1) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1932_value_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c `cloud_storage_cache_size` (coalesce nil))))) "r") -}}
{{- $value := (index $_1932_value_ok 0) -}}
{{- $ok := (index $_1932_value_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
{{- break -}}
