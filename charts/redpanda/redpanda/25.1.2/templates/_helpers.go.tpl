...
{{- $cert := (get (fromJson (include "redpanda.TLSCertMap.MustGet" (dict "a" (list (deepCopy $values.tls.certs) $name)))) "r") -}}
...
{{- $_287_result_err := (list (semverCompare $constraint $version) nil) -}}
{{- $result := (index $_287_result_err 0) -}}
{{- $err := (index $_287_result_err 1) -}}
...
{{- $_is_returning := false -}}
{{- $untyped := $in -}}
{{- $expanded := (get (fromJson (include "redpanda.recursiveTpl" (dict "a" (list $dot $untyped)))) "r") -}}
{{- $_is_returning = true -}}
...
{{- range $key, $value := $m -}}
{{- $_ := (set $m $key (get (fromJson (include "redpanda.recursiveTpl" (dict "a" (list $dot $value)))) "r")) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
...
{{- $out := (coalesce nil) -}}
{{- range $i, $_ := $s -}}
{{- $out = (concat (default (list) $out) (list (get (fromJson (include "redpanda.recursiveTpl" (dict "a" (list $dot (index $s $i))))) "r"))) -}}
{{- end -}}
{{- if $_is_returning -}}
{{- break -}}
{{- end -}}
...
{{- $_is_returning = true -}}
{{- (dict "r" (tpl (get (fromJson (include "_shims.typeassertion" (dict "a" (list "string" $data)))) "r") $dot)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
{{- end -}}
...
{{- $overrideByKey := (dict) -}}
{{- range $_, $el := $override -}}
{{- $_424_key_ok := (get (fromJson (include "_shims.get" (dict "a" (list $el $mergeKey)))) "r") -}}
{{- $key := (index $_424_key_ok 0) -}}
{{- $ok := (index $_424_key_ok 1) -}}
{{- if (not $ok) -}}
{{- continue -}}
{{- end -}}
...
{{- $merged := (coalesce nil) -}}
{{- range $_, $el := $original -}}
{{- $_436_key__ := (get (fromJson (include "_shims.get" (dict "a" (list $el $mergeKey)))) "r") -}}
{{- $key := (index $_436_key__ 0) -}}
{{- $_ := (index $_436_key__ 1) -}}
{{- $_ := (set $originalKeys $key true) -}}
{{- $_438_elOverride_7_ok_8 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $overrideByKey $key (coalesce nil))))) "r") -}}
{{- $elOverride_7 := (index $_438_elOverride_7_ok_8 0) -}}
{{- $ok_8 := (index $_438_elOverride_7_ok_8 1) -}}
{{- if $ok_8 -}}
...
{{- end -}}
{{- range $_, $el := $override -}}
{{- $_448_key_ok := (get (fromJson (include "_shims.get" (dict "a" (list $el $mergeKey)))) "r") -}}
{{- $key := (index $_448_key_ok 0) -}}
{{- $ok := (index $_448_key_ok 1) -}}
{{- if (not $ok) -}}
{{- continue -}}
{{- end -}}
{{- $_453___ok_9 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $originalKeys $key false)))) "r") -}}
{{- $_ := (index $_453___ok_9 0) -}}
{{- $ok_9 := (index $_453___ok_9 1) -}}
{{- if $ok_9 -}}
{{- continue -}}
{{- end -}}
