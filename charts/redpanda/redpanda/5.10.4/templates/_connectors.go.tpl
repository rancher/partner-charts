...
{{- $_ := (set $connectorsValue.connectors "brokerTLS" (get (fromJson (include "redpanda.ListenerConfig.ConnectorsTLS" (dict "a" (list $values.listeners.kafka $values.tls (get (fromJson (include "redpanda.Fullname" (dict "a" (list $dot)))) "r"))))) "r")) -}}
