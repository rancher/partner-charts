...
{{- if and .Values.services.aggregatedapis.admissionController .Values.services.aggregatedapis.mutatingAdmissionPolicy }}
        - "--feature-gates=MutatingAdmissionPolicy=true"
{{- end }}{{/* mutatingAdmissionPolicy */}}
{{/*
...
{{- end }}
        securityContext:
            ...
            readOnlyRootFilesystem: {{ .Values.features.persistentPodsReadOnlyRootFilesystem | default false }}
            capabilities:
                drop: ["ALL"]
        livenessProbe:
{{- if eq $service "aggregatedapis" }}
...
{{- end }}
        env:
{{/* Lets the logger stamp service_name on every log line: HOSTNAME alone
     cannot identify the emitter in colocated pods, where several service
     containers share one pod. */}}
          - name: SERVICE_NAME
            value: {{ $service }}
...
{{- end }}
{{- end }}
{{- if list "dashboardbff" "executor" "garbagecollector" "controllermanager" "kanister" "crypto" | has $service}}
...
{{- if .Values.workerPodCRDs.resourcesRequests.maxEphemeralStorage }}
          - name: EPHEMERAL_POD_MAX_STORAGE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: workerPodEphemeralStorage
{{- end }}
{{- end }}
{{- if eq $service "executor" }}
          ...
          - name: K10_IMPORT_LATEST_RESTORE_POINT_ONLY
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10ImportLatestRestorePointOnly
          - name: K10_EXECUTOR_PHASE_PROGRESS_UPDATE_INTERVAL
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10ExecutorPhaseProgressUpdateInterval
          - name: CSI_SNAPSHOT_CREATION_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: csiSnapshotCreationTimeout
          - name: CSI_SNAPSHOT_READY_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: csiSnapshotReadyTimeout
{{- end }}
{{- if or (eq $service "executor") (eq $service "controllermanager") (eq $service "dashboardbff") (eq $service "aggregatedapis") }}
...
{{- end }}
{{- if eq $service "aggregatedapis" }}
          - name: KASTEN_DISASTER_RECOVERY_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: kastenDisasterRecoveryTimeout
          - name: KASTEN_DISASTER_RECOVERY_VALIDATION_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: kastenDisasterRecoveryValidationTimeout
          - name: KASTEN_DISASTER_RECOVERY_CATALOG_SCALE_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: kastenDisasterRecoveryCatalogScaleTimeout
{{- end }}
...
{{- end }}
{{- if or (eq $service "kanister") (eq $service "executor") (eq $service "aggregatedapis")}}
          - name: DATA_MOVER_IMAGE
            value: {{ include "get.datamoverImage" . }}
          - name: DATA_MOVER_GRPC_MAX_RECV_MSG_SIZE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: datamoverGRPCMaxRecvMsgSize
                optional: true
{{- end }}
{{- if eq $service "controllermanager"}}
          - name: FR_SESSION_IMAGE
            value: {{ include "get.frsessionImage" . }}
{{- end }}
...
{{- end }}
          ...
          {{- if .Values.datastore.restoreReadAmplificationThreshold }}
          - name: K10_DATA_STORE_RESTORE_READ_AMPLIFICATION_THRESHOLD
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10DataStoreRestoreReadAmplificationThreshold
          {{- end }}
          {{- if .Values.datastore.restoreContentDensityThreshold }}
          - name: K10_DATA_STORE_RESTORE_CONTENT_DENSITY_THRESHOLD
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10DataStoreRestoreContentDensityThreshold
          {{- end }}
          - name: K10_DATA_STORE_RESTORE_USE_LEGACY
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10DataStoreRestoreUseLegacy
          ...
          - name: K10_LIMITER_VM_SNAPSHOTS_PER_CLUSTER
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10LimiterVMSnapshotsPerCluster
          - name: K10_LIMITER_VM_BACKUPS_PER_CLUSTER
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10LimiterVMBackupsPerCluster
          ...
          - name: K10_LIMITER_VOLUME_RETIRES_PER_CLUSTER
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10LimiterVolumeRetiresPerCluster
          ...
          - name: K10_IMAGE_STORAGE_CLASS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10ImageStorageClass
          - name: AWS_ASSUME_ROLE_DURATION
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: AWSAssumeRoleDuration
          - name: K10_CATALOG_OPERATIONS_BATCH_LIMIT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10CatalogOperationsBatchLimit
{{- if and (eq $service "insights") .Values.insights }}
          - name: K10_INSIGHTS_EXTERNAL_ID
            value: {{ .Values.insights.externalIdentifier | quote }}
          - name: K10_INSIGHTS_REMOTE_API_KEY
            valueFrom:
              secretKeyRef:
                name: securiti-dcc-credentials
                key: apiKey
          - name: K10_INSIGHTS_REMOTE_API_SECRET
            valueFrom:
              secretKeyRef:
                name: securiti-dcc-credentials
                key: secretKey
          - name: K10_INSIGHTS_REMOTE_TENANT_ID
            valueFrom:
              secretKeyRef:
                name: securiti-dcc-credentials
                key: tident
          - name: K10_INSIGHTS_REMOTE_URL
            value: {{ .Values.insights.url | quote }}
{{- end }}
{{- if (list "kanister" "executor" "repositories" "aggregatedapis" | has $service) }}
...
{{- end }}
{{- if (list "kanister" "executor" "repositories" "aggregatedapis" "controllermanager" | has $service) }}
          - name: K10_ENABLE_INDEXDB
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: k10EnableIndexDB
        ...
        {{/* The original plan to make the 'workerPodCRDs*' variables available in the
             controllermanager was to remove the 'if' block below and enable all the
             WORKER_POD* variables. However, the METRIC_SIDECAR* variables prevent FRS pods
             from starting so can't be enabled for the controllermanager. */}}
...
{{- end }}
{{- if (list "kanister" "executor" "repositories" "aggregatedapis" | has $service) }}
          {{/* Don't enable METRIC_SIDECAR* variables in the controllermanager - see comment
               above */}}
        ...
        {{- if or (.Values.workerPodMetricSidecar.resources.requests.memory) (.Values.kanisterPodMetricSidecar.resources.requests.memory) (.Values.global.resources.requests.memory) }}
        ...
        {{- end }}
        {{- if or (.Values.workerPodMetricSidecar.resources.requests.cpu) (.Values.kanisterPodMetricSidecar.resources.requests.cpu) (.Values.global.resources.requests.cpu) }}
        ...
        {{- end }}
        {{- if or (index .Values.workerPodMetricSidecar.resources.requests "ephemeral-storage") (index .Values.kanisterPodMetricSidecar.resources.requests "ephemeral-storage") (index .Values.global.resources.requests "ephemeral-storage") }}
          - name: K10_WORKER_POD_METRIC_SIDECAR_EPHEMERAL_STORAGE_REQUEST
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: WorkerPodMetricSidecarEphemeralStorageRequest
        {{- end }}
        {{- if or (.Values.workerPodMetricSidecar.resources.limits.memory) (.Values.kanisterPodMetricSidecar.resources.limits.memory) (.Values.global.resources.limits.memory) }}
        ...
        {{- end }}
        {{- if or (.Values.workerPodMetricSidecar.resources.limits.cpu) (.Values.kanisterPodMetricSidecar.resources.limits.cpu) (.Values.global.resources.limits.cpu) }}
        ...
        {{- end }}
        {{- if or (index .Values.workerPodMetricSidecar.resources.limits "ephemeral-storage") (index .Values.kanisterPodMetricSidecar.resources.limits "ephemeral-storage") (index .Values.global.resources.limits "ephemeral-storage") }}
          - name: K10_WORKER_POD_METRIC_SIDECAR_EPHEMERAL_STORAGE_LIMIT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: WorkerPodMetricSidecarEphemeralStorageLimit
        {{- end }}
...
{{- if (eq $service "executor") }}
          ...
          - name: K10_TIMEOUT_BACKUP
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10TimeoutBackup
          - name: K10_TIMEOUT_RESTORE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10TimeoutRestore
          - name: K10_TIMEOUT_RETIRE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10TimeoutRetire
          - name: K10_TIMEOUT_BLUEPRINT_RESOURCE_HOOKS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10TimeoutBlueprintResourceHooks
...
{{- end }}
{{- if .Values.timeout.imageStreamImageResolveSec }}
          - name: K10_IMAGESTREAM_IMAGE_RESOLVE_TIMEOUT_SEC
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10ImagestreamImageResolveTimeoutSec
{{- end }}
...
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.idleTTL }}
          - name: K10_SHARED_REPOSITORY_SERVER_IDLE_TTL
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerIdleTTL
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.reconcileTimeout }}
          - name: K10_SHARED_REPOSITORY_SERVER_RECONCILE_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerReconcileTimeout
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.serverRefreshTimeout }}
          - name: K10_SHARED_REPOSITORY_SERVER_REFRESH_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerRefreshTimeout
{{- end }}
{{- if and (list "controllermanager" "executor" | has $service) .Values.sharedRepositoryServer.startTimeout }}
          - name: K10_SHARED_REPOSITORY_SERVER_START_TIMEOUT
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerStartTimeout
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.contentCacheSizeMB }}
          - name: K10_SHARED_REPOSITORY_SERVER_CONTENT_CACHE_SIZE_MB
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerContentCacheSizeMB
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.metadataCacheSizeMB }}
          - name: K10_SHARED_REPOSITORY_SERVER_METADATA_CACHE_SIZE_MB
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerMetadataCacheSizeMB
{{- end }}
{{- if and (eq $service "controllermanager") .Values.sharedRepositoryServer.totalCacheSizeLimitMB }}
          - name: K10_SHARED_REPOSITORY_SERVER_TOTAL_CACHE_SIZE_LIMIT_MB
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: K10SharedRepositoryServerTotalCacheSizeLimitMB
{{- end }}
          ...
          - name: K10_API_DOMAIN
            valueFrom:
              configMapKeyRef:
                ...
                key: apiDomain
{{- end }}
{{- if dig "opentelemetry" "traces" "enabled" false (.Values | merge (dict)) }}
          {{- include "k10.otelTraceEnv" . | trim | nindent 10 }}
{{- end }}
...
{{- with (include "k10.cacertconfigmapname" .) }}
          - name: CACERT_CONFIGMAP_NAME
            value: {{ . }}
          - name: CACERT_CONFIGMAP_KEY
            value: {{ include "k10.cacertconfigmapkey" $main_context }}
{{- end }}
...
{{- if or (.Values.genericVolumeSnapshot.resources.requests.memory) (.Values.global.resources.requests.memory) }}
...
{{- end }}
{{- if or (.Values.genericVolumeSnapshot.resources.requests.cpu) (.Values.global.resources.requests.cpu) }}
...
{{- end }}
{{- if or (index .Values.genericVolumeSnapshot.resources.requests "ephemeral-storage") (index .Values.global.resources.requests "ephemeral-storage") }}
          - name: KANISTER_TOOLS_EPHEMERAL_STORAGE_REQUESTS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: KanisterToolsEphemeralStorageRequests
{{- end }}
{{- if or (.Values.genericVolumeSnapshot.resources.limits.memory) (.Values.global.resources.limits.memory) }}
...
{{- end }}
{{- if or (.Values.genericVolumeSnapshot.resources.limits.cpu) (.Values.global.resources.limits.cpu) }}
...
{{- end }}
{{- if or (index .Values.genericVolumeSnapshot.resources.limits "ephemeral-storage") (index .Values.global.resources.limits "ephemeral-storage") }}
          - name: KANISTER_TOOLS_EPHEMERAL_STORAGE_LIMITS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: KanisterToolsEphemeralStorageLimits
{{- end }}
{{- end }}
{{- if (list "dashboardbff" "controllermanager" "executor" | has $service) }}
    ...
    {{- else -}}
        ...
        {{- end -}}
    {{- end }}
        {{- if or (.Values.grafana.link) (.Values.grafana.external.url) }}
          - name: GRAFANA_URL
...
{{- if eq $service "controllermanager" }}
    ...
    {{- end }}
          - name: K10_FRS_MAX_MOUNTS_PER_SESSION
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: frsMaxMountsPerSession
          - name: K10_FRS_MAX_MOUNTS_PER_NAMESPACE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: frsMaxMountsPerNamespace
          - name: K10_FRS_MAX_SYSTEM_MOUNTS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: frsMaxSystemMounts
          - name: K10_FRS_SESSION_EXPIRY_MINS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: frsSessionExpiryTimeInMinutes
{{- end -}}
...
{{- if eq $service "executor" }}
        - name: datastore-cache-vol
          mountPath: "/tmp"
{{- end }}
{{- if and (.Values.features.persistentPodsReadOnlyRootFilesystem) (list "state" "aggregatedapis" "auth" "catalog" "controllermanager" "crypto" "dashboardbff" "garbagecollector" "jobs" "kanister" "metering" "repositories" "vbrintegrationapi" "logging" "frontend" | has $service) }}
        - name: tmp-vol
          mountPath: "/tmp"
{{- end }}
{{- if and (eq $service "controllermanager") (or .Values.injectGenericVolumeBackupSidecar.enabled .Values.injectKanisterSidecar.enabled) }}
        - name: mutating-webhook-certs
          ...
          readOnly: true
{{- end }}
{{- if list "dashboardbff" "auth" "controllermanager" "vbrintegrationapi" | has $service}}
...
{{- with (include "k10.cacertconfigmapname" .) }}
        - name: {{ . }}
          mountPath: {{ printf "/etc/ssl/certs/%s" (include "k10.cacertconfigmapkey" $main_context) }}
          subPath: {{ include "k10.cacertconfigmapkey" $main_context }}
{{- end }}
{{- if eq $service "frontend" }}
        ...
        - name: frontend-config
          ...
          readOnly: true
{{- if .Values.features.persistentPodsReadOnlyRootFilesystem }}
        - name: nginx-writable-vol
...
{{- if and (eq $service "aggregatedapis") (include "k10.siemEnabled" .) }}
        - name: aggauditpolicy-config
          ...
          readOnly: true
{{- end}}
{{- if and (.Values.features.persistentPodsReadOnlyRootFilesystem) (eq $service "aggregatedapis") }}
        - name: var-run-vol
          mountPath: /var/run
{{- end}}
