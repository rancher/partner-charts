...
metadata:
  ...
  labels:
{{ include "k10.resourceAndHelmLabels" $main_context | indent 4 }}
...
{{- if eq $service "logging" -}}
  {{- $_ := set $requiredAnnotations "checksum/fluentbit-config" (include (print .Template.BasePath "/fluentbit-configmap.yaml") . | sha256sum) -}}
{{- end -}}
{{- if .Values.auth.ldap.restartPod -}}
  {{- $_ := set $requiredAnnotations "rollme" (randAlphaNum 5) -}}
{{- end -}}
        {{- with merge (dict "requiredAnnotations" $requiredAnnotations "k10Service" $service) . }}
...
-}}
{{- if list "executor" "controllermanager" "aggregatedapis" "dashboardbff" "crypto" "catalog" "kanister" | has $service}}
...
{{- if list "executor" | has $pod}}
      - name: datastore-cache-vol
        emptyDir: {}
{{- end }}
{{- if and (.Values.features.persistentPodsReadOnlyRootFilesystem) (list "aggregatedapis" "state" "auth" "catalog" "controllermanager" "crypto" "dashboardbff" "garbagecollector" "jobs" "kanister" "metering" "repositories" "vbrintegrationapi" "logging" "frontend" | has $pod) }}
      - name: tmp-vol
        emptyDir: {}
{{- end }}
{{- if and (.Values.features.persistentPodsReadOnlyRootFilesystem) (eq $pod "frontend") }}
      - name: nginx-writable-vol
        emptyDir: {}
{{- end }}
{{- if and (.Values.features.persistentPodsReadOnlyRootFilesystem) (eq $pod "aggregatedapis") }}
      - name: var-run-vol
        emptyDir: {}
{{- end }}
