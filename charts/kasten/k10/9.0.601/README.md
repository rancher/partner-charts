...
`global.resourceLabels` | Configures custom labels to be set on all Kasten resources | `None`
`global.ephemeralResourceLabels` | Configures custom labels to be set on all Kasten ephemeral resources | `None`
...
`global.resources.[requests\|limits]` | Global resource requests and limits for K10 pods deployed by the chart | `None`
...
`global.acm.enabled` | Enable Red Hat Advanced Cluster Management (ACM) Observability integration | `false`
`global.acm.hubThanosTenantId` | The Thanos Tenant ID for the ACM Hub (required if enabled) | `''`
`global.acm.managedClusterId` | The Cluster ID of the managed cluster. If not set, auto-detection is attempted. | `''`
`global.acm.tls.clientCertSecretName` | Name of a Kubernetes Secret containing the client TLS certificate and key (`tls.crt`, `tls.key`) for mTLS authentication to the ACM Hub | `''`
`global.acm.tls.serverCAConfigMapName` | Name of a ConfigMap containing the server CA certificate (`ca.crt`) for verifying the ACM Hub endpoint. Requires `clientCertSecretName` | `''`
`global.acm.tls.insecureSkipVerify` | Skip TLS server certificate verification when mTLS is enabled. Requires `clientCertSecretName` | `false`
...
`prometheus.server.remote_write[].dashboardUrl` | Optional K10 Dashboard URL to be injected as a label in ACM metrics | `''`
`prometheus.server.remote_write[].metricsRegex` | Optional regex to filter metrics sent to ACM (only matching metrics are sent) | `''`
...
`gateway.resources.[requests\|limits].[cpu\|memory\|ephemeral-storage]` | Resource requests and limits for gateway Pod | `{}`
`gateway.service.externalPort` | Specifies the gateway services external port | `80`
`genericVolumeSnapshot.resources.[requests\|limits].[cpu\|memory\|ephemeral-storage]` | Specifies resource requests and limits for generic backup sidecar and all temporary Kasten worker Pods. Superseded by ActionPodSpec | `{}`
...
`timeout.backup` | Specifies the maximum duration (in minutes) to complete any of the following operations: Blueprint backup action; Generic storage backup (per PVC); Volume snapshot export (per PVC) | `45`
`timeout.restore` | Specifies the maximum duration (in minutes) to complete any of the following operations: Blueprint restore action; Volume restore (per PVC) | `600`
`timeout.retire` | Specifies the maximum duration (in minutes) to complete any of the following operations: Blueprint delete action; Retire exported volume data (per PVC) | `45`
`timeout.blueprintResourceHooks` | Specifies the timeout (in minutes) for Blueprint backupPrehook, backupPosthook, and restorePosthook actions | `20`
...
`encryption.primaryKey.azureKeyVaultKeyName` | Specifies the Azure Key Vault key name for encrypting K10 Primary Key | `None`
`encryption.primaryKey.azureKeyVaultURL` | Specifies the Azure Key Vault URL which holds the key for encrypting K10 Primary Key | `None`
...
`executor.phaseProgressUpdateInterval` | Controls how often phase progress is updated during backup, restore, and export operations. Accepts [Go duration format](https://pkg.go.dev/time#ParseDuration) (e.g. `"5s"`, `"10s"`). Minimum `5s`. | `"10s"`
`executor.csiSnapshotCreationTimeout` | Maximum time to wait for a CSI provisioner to complete point-in-time capture. Status errors are ignored until this timeout elapses, allowing asynchronous or proxy CSI drivers to finish materializing the backend snapshot. Accepts [Go duration format](https://pkg.go.dev/time#ParseDuration) (e.g. `"10m"`, `"1h"`). Minimum `1m`. | `"10m"`
`executor.csiSnapshotReadyTimeout` | Maximum time to wait for a CSI snapshot to become ready (VS.Status.ReadyToUse=true). If the snapshot does not become ready within this window the backup fails. Accepts [Go duration format](https://pkg.go.dev/time#ParseDuration) (e.g. `"30m"`, `"1h"`). Minimum `1m`. | `"30m"`
...
`workerPodMetricSidecar.resources.[requests\|limits].[cpu\|memory\|ephemeral-storage]` | Specifies resource requests and limits for the temporary worker Pod metric sidecar | `{}`
...
`datastore.restoreReadAmplificationThreshold` | Number of MiB read per content during restore | `null`
`datastore.restoreContentDensityThreshold` | Number of contents per MiB of stored data | `null`
`datastore.restoreUseLegacy` | Whether to use legacy restore mode | `false`
`kastenDisasterRecovery.quickMode.enabled` | Enables K10 Quick Disaster Recovery | `false`
`kastenDisasterRecovery.validationTimeout` | Specifies the timeout (in minutes) for validation during disaster recovery | `30`
`kastenDisasterRecovery.catalogScaleTimeout` | Specifies the timeout (in minutes) for the catalog to scale back up (including any schema-migration init container) during disaster recovery | `30`
...
`workerPodCRDs.resourcesRequests.maxEphemeralStorage` | Max storage space which may be allocated for each `ActionPodSpec` | `''`
