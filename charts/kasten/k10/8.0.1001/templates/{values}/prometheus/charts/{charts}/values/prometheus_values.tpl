...
{{- /* Use global resource labels if defined, otherwise use global pod labels, and then overwrite with any prometheus-specific labels */ -}}
{{- $resourceLabels := .Values.global.resourceLabels | default dict -}}
{{- $podLabels := .Values.global.podLabels | default dict -}}
{{- $prometheusPodLabels := .Values.prometheus.server.podLabels | default dict -}}
{{- $globalLabels := ternary $resourceLabels $podLabels (gt (len $resourceLabels) 0) -}}
{{- $allPodLabels := merge (dict) $prometheusPodLabels $globalLabels -}}

{{- /* Merge global resources with any prometheus-specific resources, where the latter is of highest priority */ -}}
{{- $serverResources := merge (dict) (.Values.prometheus.server.resources | default dict) (.Values.global.resources | default dict) -}}
{{- $configMapReloadResources := merge (dict) (.Values.prometheus.configmapReload.prometheus.resources | default dict) (.Values.global.resources | default dict) -}}
...
{{- $_ := mergeOverwrite .Values.prometheus.configmapReload.prometheus
  (dict
    "resources" $configMapReloadResources
  )
-}}
