...
{{- end }}
{{- if eq $service "controllermanager"}}
          - name: FLR_SESSION_IMAGE
            value: {{ include "get.flrsessionImage" . }}
{{- end }}
...
{{- if eq $service "controllermanager" }}
    ...
    {{- end }}
    {{- if .Values.features.snapshotContentRequestCR }}
          - name: K10_FLR_MAX_MOUNTS_PER_REQUEST
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: flrMaxMountsPerRequest
          - name: K10_FLR_MAX_MOUNTS_PER_NAMESPACE
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: flrMaxMountsPerNamespace
          - name: K10_FLR_MAX_SYSTEM_MOUNTS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: flrMaxSystemMounts
          - name: K10_FLR_SESSION_EXPIRY_MINS
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: flrSessionExpiryTimeInMinutes
    {{- end }}
{{- end -}}
