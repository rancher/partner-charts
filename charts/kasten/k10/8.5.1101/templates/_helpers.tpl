...
{{- define "k10.capabilities" -}}
  ...
  {{- if .Values.services.aggregatedapis.admissionController -}}
    {{- $internal_capabilities = append $internal_capabilities "admissioncontroller" -}}
  {{- end -}}

  {{- /* HTTP Metrics: enabled by default; disabled only when explicitly set to false */ -}}
  {{- if dig "opentelemetry" "metrics" "httpInstrumentation" true (.Values | merge (dict)) -}}
    {{- $internal_capabilities = append $internal_capabilities "opentelemetry.metrics.http" -}}
  {{- end -}}

  {{- /* Secure Cookies */ -}}
  {{- if .Values.auth.secureCookies -}}
    {{- $internal_capabilities = append $internal_capabilities "auth.securecookies" -}}
  {{- end -}}
...
{{- define "k10.capabilities_mask" -}}
  ...
  {{- /* Secure Cookies */ -}}
  {{- if not .Values.auth.secureCookies -}}
    {{- $internal_capabilities_mask = append $internal_capabilities_mask "auth.securecookies" -}}
  {{- end -}}
...
NOTE: This is primarily consumed as "k10.deploymentPodAnnotations" (see below)
*/}}
{{- define "k10.commonPodAnnotations" -}}
  {{- $serviceMeshAnnotations := dict -}}
  {{- if .k10Service -}}
    {{- $serviceMeshAnnotations = include "k10.serviceMeshAnnotations" (dict "main" . "service" .k10Service) | fromYaml -}}
  {{- end -}}
  {{- (merge (dict) (.requiredAnnotations) (dict "checksum/config" (include (print .Template.BasePath "/k10-config.yaml") . | sha256sum)) (dict "checksum/secret" (include (print .Template.BasePath "/secrets.yaml") . | sha256sum)) (include "k10.globalPodAnnotations" . | fromYaml) $serviceMeshAnnotations) | toYaml -}}
{{- end -}}


{{/* The difference between this and "k10.commonPodAnnotations" above is that this also includes SCC
annotations for OpenShift clusters which cannot be included in helm hook podTemplates.

NOTE: The contents of this are primarily defined above in "k10.commonPodAnnotations"
*/}}
{{- define "k10.deploymentPodAnnotations" -}}
  {{- (merge (dict) ( include "k10.commonPodAnnotations" . | fromYaml) (include "k10.sccAnnotations" . | fromYaml)) | toYaml -}}
{{- end -}}
...
{{- $primaryKey := (.Values.encryption | default dict).primaryKey | default dict }}
{{- if (or (or $primaryKey.awsCmkKeyId $primaryKey.vaultTransitKeyName) (and $primaryKey.azureKeyVaultKeyName $primaryKey.azureKeyVaultURL)) -}}
{{- print true -}}
{{- end -}}
{{- end -}}
...
{{/*
Returns OpenTelemetry tracing environment variables for the OTLP exporter.
Uses dig throughout so missing intermediate keys in .Values do not cause nil errors.
Caller is expected to gate on dig "opentelemetry" "traces" "enabled" and to
indent the output to match the surrounding env list.
*/}}
{{- define "k10.otelTraceEnv" }}
{{- $v := .Values | merge (dict) -}}
{{- $endpoint := dig "opentelemetry" "traces" "tracesEndpoint" "" $v -}}
{{- $sampler := dig "opentelemetry" "traces" "tracesSampler" "parentbased_traceidratio" $v -}}
{{- $samplerArg := dig "opentelemetry" "traces" "tracesSamplerArg" "0.01" $v -}}
{{- if not $endpoint }}
...
{{- define "k10.grafanaUrl" -}}
  {{- if .Values.grafana.link -}}
    {{- .Values.grafana.link -}}
  {{- else if .Values.grafana.external.url -}}
...
{{/*
Generate service mesh annotations for aggregated API server pods.
The aggregated API server requires special handling in a service mesh because:
1. The Kubernetes API server needs to communicate directly with it using its own TLS certificates
2. Istio/Linkerd sidecars would intercept this traffic and cause TLS handshake failures
3. We exclude the aggregated API ports from sidecar interception to allow direct communication

This helper detects the service mesh type and applies appropriate annotations.
For service argument, pass the service name (e.g., "aggregatedapis")
*/}}
{{- define "k10.serviceMeshAnnotations" -}}
  {{- $service := .service -}}
  {{- with .main -}}
    {{- if eq $service "aggregatedapis" -}}
      {{- $meshType := "" -}}
      {{- if .Values.global.serviceMesh -}}
        {{- $meshType = .Values.global.serviceMesh.type -}}
      {{- end -}}
      {{- if eq $meshType "auto" -}}
        {{- if .Capabilities.APIVersions.Has "networking.istio.io/v1beta1" -}}
          {{- $meshType = "istio" -}}
        {{- else if .Capabilities.APIVersions.Has "policy.linkerd.io/v1beta1" -}}
          {{- $meshType = "linkerd" -}}
        {{- end -}}
      {{- end -}}
      {{- if eq $meshType "istio" }}
traffic.sidecar.istio.io/excludeInboundPorts: "443,10250"
      {{- else if eq $meshType "linkerd" }}
config.linkerd.io/skip-inbound-ports: "443,10250"
      {{- end -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
...
{{/*
check if securiti dcc reporting is enabled; only enabled if all required values are provided and airgapped repository is not being used
*/}}
{{- define "k10.insights.enabled" -}}
{{- if and .Values.insights .Values.insights.apiKey .Values.insights.secretKey .Values.insights.url .Values.insights.tident (not .Values.global.airgapped.repository) -}}
{{- print true -}}
{{- end -}}
{{- end -}}
