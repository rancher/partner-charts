...
K10 NOTE:
  ...
  with these values. However, we have always shipped with pushgateway disabled.

  If a customer had explicitly enabled node-experter, a change to these selector
  labels will mean that all customers must manually delete the Statefulset
  before upgrading, which is a situation we don't want for our customers.

  Instead, the `app.kubernetes.io/name` and `app.kubernetes.io/instance` labels
  are included in the `prometheus-pushgateway.defaultLabels` block above.
