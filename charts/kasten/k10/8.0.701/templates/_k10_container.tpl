...
{{- if (list "kanister" "executor" "repositories" "aggregatedapis" | has $service) }}
        ...
        {{- end }}
        {{- if or (not (quote (index .Values.workerPodMetricSidecar.resources.requests "ephemeral-storage") | empty)) (not (quote (index .Values.kanisterPodMetricSidecar.resources.requests "ephemeral-storage") | empty)) }}
        ...
        {{- end }}
        {{- if or (not (quote (index .Values.workerPodMetricSidecar.resources.limits "ephemeral-storage") | empty)) (not (quote (index .Values.kanisterPodMetricSidecar.resources.limits "ephemeral-storage") | empty)) }}
...
{{- if eq $service "controllermanager" }}
    ...
    {{- if .Values.features.fileRecoverySessionCR }}
          - name: K10_FRS_MAX_MOUNTS_PER_REQUEST
            valueFrom:
              configMapKeyRef:
                name: k10-config
                key: frsMaxMountsPerRequest
