...
{{- if .Values.metering.reportingKey }}
    - name: servicecontrol
      servicecontrol:
        identity: gcp
        serviceName: veeam-kasten-google-marketplace.endpoints.veeam-marketplace-public.cloud.goog
...
{{ include "k10.common.matchLabels" . | indent 6 }}
  ...
  template:
    metadata:
      ...
      labels:
        {{- with merge (dict "requiredLabels" (dict "app.kubernetes.io/component" $service "component" $service "run" (printf "%s-svc" $service) )) . }}
...
{{- else }}
          ...
          - name: NODE_COMPLIANCE_STORE
            value: /tmp/reports/node_compliance
{{- end }}
{{- if .Values.metering.awsRegion }}
          - name: AWS_REGION
            value: {{ .Values.metering.awsRegion }}
{{- end }}
{{- if eq (include "check.azMarketplaceInstallation" .) "true"}}
          - name: AZURE_MP_CLIENT_ID
            value: {{ include "get.ClientIdForAzureMarketplace" . }}
{{- end }}
