...
{{- /*** GRAFANA LABELS ***/ -}}
{{- /* Merge global pod labels with any grafana-specific labels, where the latter is of highest priority */ -}}
{{- $podLabels := merge (dict) (dict "component" "grafana") (.Values.grafana.podLabels | default dict) (.Values.global.podLabels) -}}

{{- /* Merge global pod annotations with any grafana-specific annotations, where the latter is of highest priority */ -}}
{{- $podAnnotations := merge (dict) (.Values.grafana.podAnnotations | default dict) (.Values.global.podAnnotations) -}}
{{- if .Values.scc.create -}}
  {{- $podAnnotations = merge (dict "openshift.io/required-scc" (printf "%s-grafana" .Release.Name)) $podAnnotations -}}
{{- end -}}
