...
![Version: 0.44.3](https://img.shields.io/badge/Version-0.44.3-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.44.3](https://img.shields.io/badge/AppVersion-0.44.3-informational?style=flat-square)
...
### ArgoCD
If you are using ArgoCD to manage your Kubernetes applications, helm autogenerated certificates are not fully supported.
Please make sure to set `admissionWebhooks.autoGenerateCert.enabled` to `false` and `admissionWebhooks.autoGenerateCert.recreate` to `false `in your `values.yaml` file and follow the steps in the `cert-manager` section or provide your own certificate and key files.
