...
![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![Version: 2.14.3](https://img.shields.io/badge/Version-2.14.3-informational?style=flat-square) ![AppVersion: 2.14.3](https://img.shields.io/badge/AppVersion-2.14.3-informational?style=flat-square)
...
| kubectl.image.tag | string | `"v1.36.1@sha256:d08f476d04d0e30f426f06bc6ff6c38913aaa4591943046b77e2f74a72d3611c"` | The kubectl image tag |
...
| experimental.exposeZoneProxyMetrics | bool | `false` | If true, the Envoy admin listener on zone proxies also serves /stats/prometheus without mTLS, so Prometheus can scrape Envoy stats off the proxy IP. Only that path is added; the rest of the Admin API still requires mTLS. Useful for scraping ZoneIngress/ZoneEgress, which MeshMetric does not apply to. Note the stats dump lists every upstream cluster name, so effectively every service in the mesh, and Envoy renders it on the main dispatcher thread, so unauthenticated request loops cost CPU on a busy zone ingress. Restrict the admin port with a NetworkPolicy scoped to Prometheus. |
